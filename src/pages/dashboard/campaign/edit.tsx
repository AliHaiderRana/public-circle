...
// Enhanced schema matching create form
      ...
      getCampaignById(id)
        .then(async (response) => {
          if (response?.data?.data) {
            ...
            setCampaignCompanyId(campaign.campaignCompanyId || '');

            // Parse recurring period
            let duration = '';
            let unit: 'minute' | 'hour' | 'day' | 'month' | 'year' = 'hour';
            if (campaign.recurringPeriod) {
              const parts = campaign.recurringPeriod.split(' ');
              duration = parts[0] || '';
              unit = (parts[1] as any) || 'hour';
            }
            setDurationValue(duration);
            setTimeUnit(unit);

            // Set CC/BCC emails
            setCcEmails(campaign.cc || []);
            setBccEmails(campaign.bcc || []);

            // Fetch and set template
            if (campaign.emailTemplate || campaign.emailTemplateId) {
              const templateId = campaign.emailTemplate || campaign.emailTemplateId;
            ...
            if (campaign.attachments && Array.isArray(campaign.attachments)) {
              const attachmentFiles: AttachmentFile[] = campaign.attachments.map((att: any) => ({
                id: att._id || att.id,
            ...
            if (campaign.runSchedule) {
              runSchedule = dayjs(campaign.runSchedule).format('YYYY-MM-DDTHH:mm');
            }

            // Determine run mode
            const runMode = campaign.runSchedule ? 'SCHEDULE' : 'INSTANT';

            // Determine frequency
            const frequency = campaign.frequency === 'MANY_TIMES' ? 'MANY_TIMES' : 'ONE_TIME';

            // Set end date if available
            let endDateValue = undefined;
            if (campaign.endDate) {
              endDateValue = dayjs(campaign.endDate).format('YYYY-MM-DD');
            ...
            form.reset({
              ...
              companyGroupingId: campaign.companyGroupingId?._id || campaign.companyGroupingId || '',
              sourceEmailAddress: campaign.sourceEmailAddress || '',
              emailSubject: campaign.emailSubject || '',
              emailTemplateId: campaign.emailTemplate || campaign.emailTemplateId || '',
              ...
              recurringPeriod: campaign.recurringPeriod || undefined,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Update campaign details and settings</p>
        </div>
      </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Campaign ID" {...field} value={campaignCompanyId || field.value} readOnly />
                    </FormControl>
                    <FormDescription>Unique identifier for this campaign (cannot be changed)</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
      ...
      <TemplatePreviewDrawer
        ...
        templateId={selectedTemplate?._id || form.watch('emailTemplateId')}
        htmlContent={selectedTemplate?.body || selectedTemplate?.html}
      />
