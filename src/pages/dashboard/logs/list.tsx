...
import { RefreshCw, Eye, Search, ChevronLeft, ChevronRight, BarChart3, Users, Calendar, Loader2 } from 'lucide-react';
...
export default function CampaignLogsPage() {
  ...
  const formatDateTime = (date: string | Date | null | undefined, time?: string | Date | null | undefined) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {isLoading ? 'Loading...' : `${totalRecords || 0} Campaign Log${(totalRecords || 0) !== 1 ? 's' : ''}`}
          </CardTitle>
        </CardHeader>
        <CardContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    {displayLogs.map((log: CampaignLog) => {
                      const campaignName = log.emailSubject || log.campaignName || '—';
                          ...
                          <TableCell className="text-center">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <span className="text-sm">
                                {formatDateTime(log.createdAt, log.lastProcessed)}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell className="text-center">
                          ...
                          <TableCell className="text-right">
                            <TooltipProvider>
                              <Tooltip>
                                ...
                                <TooltipContent>
                                  {isDisabled ? 'No runs available' : 'View Campaign Runs'}
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          </TableCell>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <div className="text-sm text-muted-foreground">
                    Page {page} of {totalPages} • {totalRecords || 0} total log{(totalRecords || 0) !== 1 ? 's' : ''}
                  </div>
                  <div className="flex items-center gap-2">
                    <Select
                      value={pageSize.toString()}
                      onValueChange={(value) => handleChangeRowsPerPage(parseInt(value, 10))}
                    >
