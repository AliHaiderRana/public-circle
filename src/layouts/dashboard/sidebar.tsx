          ...
          >
            <div className="rounded-md border bg-popover overflow-hidden">
              ...
              {item.children?.map((child) => {
                ...
                const childIsAllowed =
                  !isSubscriptionCancelled ||
                  !child.path ||
                  ALLOWED_PATHS_WHEN_CANCELLED.some(
                    (allowedPath) =>
                      child.path === allowedPath ||
                      child.path?.startsWith(allowedPath + "/"),
                  );
    ...
    <SidebarHeader>
      ...
      >
        {isCollapsed ? (
          ...
          <Logo isSingle={false} width={160} height={34} disableLink={false} />
        )}
      </div>
    </SidebarHeader>
  );
